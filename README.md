...
Khác với các công cụ OCR truyền thống (như Tesseract) thường làm mất định dạng và nối dòng lộn xộn, phần mềm này ứng dụng mô hình AI Đa phương thức (Google Gemini 1.5 Flash / 3.1) để "đọc hiểu" ngữ cảnh trang giấy, khôi phục cấu trúc và xuất ra file Word hoàn chỉnh.
...
![Giao diện chính](link_den_anh_giao_dien_chinh.png)
*Giao diện làm việc với chế độ Đơn và Hàng loạt*

![Tiện ích gộp ảnh](link_den_anh_popup.png)
...
2. Chạy lệnh Build:
   ```bash
   pyinstaller --noconsole --windowed --onefile bot_gui.py
   ```
